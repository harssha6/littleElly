...
const CenterPill = (props) => {
  ...
  return (
    <div
      onClick={onClick}
      className="relative cursor-pointer border group hover:bg-[linear-gradient(180deg,_#FFF_0%,_#FFF3F3_100%)] hover:border-red-200 border-[rgba(113,101,101,0.21)] px-7 py-[18px] rounded-[10px] w-[180px] max-md:w-[47%] h-[90px] flex flex-row items-center justify-center"
    >
      <Text
        type="base"
        className="font-[400] text-[#1c1c1c] group-hover:text-primary text-center"
      >
        {label}
      </Text>
      <div className="w-[24px] h-[24px] transition-all duration-500 opacity-0 right-[20%] pointer-events-none absolute group-hover:right-[4%] group-hover:opacity-100">
        ...
        >
          <path
            d="M4.00049 12.9265L20.0005 12.9265M20.0005 12.9265L14.0005 6.92648M20.0005 12.9265L14.0005 18.9265"
            stroke="#EE7F82"
            strokeWidth="2"
            strokeLinecap="round"
...
const FindCentre = (props) => {
  ...
  const getState = (district) => {
    return centers.find((center) =>
      center.districts?.find((d) => d.district === district)
    )?.state;
  };
  ...
  const renderPill = (pills) => {
    const pillsData = getPillsData(pills).map((pill) => ({
      ...
      type: pill?.state ? "state" : pill?.district ? "district" : "centre",
    }));
    ...
    return (
      <div class="w-full">
        {pillsDataGroupByPoweredByBoolean["notPoweredBy"] &&
        ...
        {pillsDataGroupByPoweredByBoolean["poweredBy"] &&
        pillsDataGroupByPoweredByBoolean["poweredBy"].length > 0 ? (
          <div>
            <img src={poweredBy.src} alt="Powered By" class="w-[145px] py-6" />
            <div className="flex flex-row flex-wrap gap-4 w-full items-center">
              {console.log(
                "pillsDataGroupByPoweredByBoolean",
                pillsDataGroupByPoweredByBoolean["poweredBy"]
              )}
          ...
          {groupKeys.map((type) => (
            <div className="py-2">
              ...
              <div className="w-full flex flex-row flex-wrap gap-4 items-center">
                {renderPill(
                  groupByType[type].map((name, index) => ({
                    label: name,
                    type,
                  }))
                )}
  ...
  let districts =
    centers.find((center) => center?.state === currentState)?.districts || [];

  let thisDistrict = districts.find(
    (district) => district?.district === currentDistrict
  ) || { centres: [] };
  ...
  return (
    <div className="flex flex-col gap-l items-center findcenter p-[60px] px-[90px] max-md:px-[20px] max-md:py-[40px]" id='find-a-center' >
      ...
      <div
        data-appear={1}
        className="relative w-full max-w-[844px] gap-2 flex flex-row justify-end py-[6px]  pr-[10px] border-[1px] border-[#ddd] rounded-full"
        style={{
          boxShadow:
            "0px 1.366px 2.732px 0px rgba(0, 0, 0, 0.08), 0px 4.098px 10px 0px rgba(0, 0, 0, 0.10)",
        }}
      >
        <input
      ...
      <div
        data-appear={2}
        className="w-full  px-[30px] py-6 bg-white border rounded-[12px] border-black/10"
      >
        {!isSearching && (
          <Text type="base">
              ...
              {currentState && (
                <>
                  ...
                  <div
                    ...
                    className={
                      "cursor-pointer " +
                      (isDistrictView ? activeBreadCrumbClass : "")
                    }
                  >
                    {currentState}
                  </div>
              ...
              {currentDistrict && (
                <>
                  ...
                  <div
                    className={
                      "cursor-pointer " +
                      (isCentreView ? activeBreadCrumbClass : "")
                    }
                  >
                    {currentDistrict}
                  </div>
            ...
            <div className="font-[600] flex flex-row text-[#7b7b7b]">
              <div
                ...
                className={
                  ...
                  (isStateView ? activeBreadCrumbClass : "")
                }
              >
                Locations
              </div>
        ...
        {isStateView &&
          renderCentres(
            centers.map((center) => ({ label: center.state, type: "state" }))
          )}
        {isSearching &&
          renderCentres(
        ...
        {isCentreView &&
          renderCentres(
            ellyCenters.map((centre) => ({ label: centre, type: "center" }))
          )}
      </div>
      <div
        data-appear={2}
        class="self-start  border-[#B6B6B6] border-l-[3px] pl-6"
      >
        <div class="p-[3px]" />
        <Text
          type="base"
        ...
        <div class="flex flex-row items-center max-md:gap-4 max-md:flex-col max-md:items-start gap-1">
          ...
          <Text
            type="base"
            className="flex flex-row max-md:flex-col items-center max-md:items-start gap-1"
          >
            <span class="font-bold">
              <span class="max-md:hidden block">-</span>
            </span>
            Schools supported by Little Elly’s curriculum and resources.
          </Text>
        </div>
        <div class="p-[3px]" />
      </div>
